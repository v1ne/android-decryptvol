...
#define property_get(x, y, z) do {(y)[0]='\0';} while(0)
#define property_set(x, y) do { } while(0)
...
static void get_device_scrypt_params(struct crypt_mnt_ftr *ftr) {
    ...
    //property_get(SCRYPT_PROP, paramstr, ""); TODO: may be different in some cases
		paramstr[0]='\0';
    if (paramstr[0] != '\0') {
        /*
...
static int get_crypt_ftr_info(char **metadata_fname, off64_t *off)
{
  ...
  if (!cached_data) {
    ...
    if (1 || !strcmp(key_loc, KEY_IN_FOOTER)) {
...
static int put_crypt_ftr_and_key(struct crypt_mnt_ftr *crypt_ftr)
{
  ...
  if (S_ISREG(statbuf.st_mode)) {
    if (ftruncate(fd, 0x4000)) {
      printf("Cannot set footer file (%s) size\n", fname);
      goto errout;
    }
  }
...
static int load_persistent_data(void)
{
    ...
    if (1 || strcmp(encrypted_state, "encrypted") ) {
        pdata = malloc(CRYPT_PERSIST_DATA_SIZE);
        if (pdata) {
            init_empty_persist_data(pdata, CRYPT_PERSIST_DATA_SIZE);
            persist_data = pdata;
...
void release_dm_dev(char *name) {
  ...
  if (ioctl(fd, DM_DEV_REMOVE, io)) {
    perror("Cannot remove dm-crypt device");
  }
}
...
again:
  if (ioctl(fd, DM_DEV_CREATE, io)) {
		if(errno == EBUSY && !ioctl(fd, DM_DEV_REMOVE, io))
			goto again;
    perror("Cannot create dm-crypt device");
    goto errout;
  }
  ...
  snprintf(crypto_blk_name, MAXPATHLEN, "/dev/mapper/%s", name);
...
int test_mount_encrypted_fs(char *passwd, char *mount_point, char *label)
{
  struct crypt_mnt_ftr crypt_ftr;
   ...
   */
  sprintf(tmp_mount_point, "%s", mount_point);
  mkdir(tmp_mount_point, 0755);
	char cmdbuf[1024];

  sprintf(cmdbuf, "mount %s %s", crypto_blkdev, tmp_mount_point);
	if(system(cmdbuf)) {
 ...
 */
int cryptfs_setup_volume(const char *label, char* dev,
...
static int cryptfs_enable_wipe(char *crypto_blkdev, off64_t size, int type)
{
    ...
    if (type == EXT4_FS) {
        ...
        num_args = 10;
        printf("Making empty filesystem with command %s %s %s %s %s %s %s %s %s %s\n",
              args[0], args[1], args[2], args[3], args[4], args[5],
    ...
    for (i=0; i<num_vols; i++) {
        if (should_encrypt(&vol_list[i])) {
            ...
            if ((ret < 0) && (ret != UNMOUNT_NOT_MOUNTED_ERR)) {
                ...
                printf("Failed to unmount volume %s\n", vol_list[i].label);
                goto error_unencrypted;
            }
        }
    }
